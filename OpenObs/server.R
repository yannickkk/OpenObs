...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    ...
    #prot_geo <- if(exists("couche_1")){leaflet(data=c(couche_1))}; if(exists("couche_2")){leaflet(data=c(couche_1,couche_2))}; if(exists("couche_3")){leaflet(data=c(couche_1,couche_2,couche_3))}; if(exists("couche_4")){leaflet(data=c(couche_1,couche_2,couche_3,couche_4))}  %>%  
    prot_geo <- leaflet() %>%
      addTiles(tilesURL) %>%
    ...
    ########Ajout couche additionnelle###############
     if(exists("couche_1")){
       prot_geo <- addPolygons(prot_geo,data=couche_1, stroke = map_stroke_couche_1, weight = map_weight_couche_1, fill = map_fill_couche_1, color = ~map_color_couche_1, smoothFactor = map_smoothFactor_couche_1, fillOpacity = as.numeric(map_fillOpacity_couche_1))
     }
     
     if(exists("couche_2")){
       prot_geo <- addPolygons(prot_geo,data=couche_2, stroke = map_stroke_couche_2, weight = map_weight_couche_2, fill = map_fill_couche_2, color = ~map_color_couche_2, smoothFactor = map_smoothFactor_couche_2, fillOpacity = as.numeric(map_fillOpacity_couche_2)) 
     }
     
     if(exists("couche_3")){
       prot_geo <- addPolygons(prot_geo,data=couche_3, stroke = map_stroke_couche_3, weight = map_weight_couche_3, fill = map_fill_couche_3, color = ~map_color_couche_3, smoothFactor = map_smoothFactor_couche_3, fillOpacity = as.numeric(map_fillOpacity_couche_3))
     }
    
     if(exists("couche_4")){
       prot_geo <- addPolygons(prot_geo,data=couche_4, stroke = map_stroke_couche_4, weight = map_weight_couche_4, fill = map_fill_couche_4, color = ~map_color_couche_4, smoothFactor = map_smoothFactor_couche_4, fillOpacity = as.numeric(map_fillOpacity_couche_4))
     }
    
    # if(exists("couche_5")){
    #   prot_geo <- addRasterImage(prot_geo,couche_5)
    # }
  ...
  observeEvent(input$species,{
    if (input$species == ""){
      ...
      url <- a(tags$h3("Plus d'information sur cette espèce"), href = as.character(unique(dat[which(dat[,wikisp_names] == name),link_names])),target = "_blank")
