...
#####################
#######fait apparaitre l'onglet protocole si si il existe un fichier html#######

if (exists('htmlf')) {
  showTab(inputId = "tabs", target =  titre_onglet4);
  output$inc <- renderUI(includeHTML(htmlf))
}

if (!exists('htmlf')) {
  hideTab(inputId = "tabs", target =  titre_onglet4)}