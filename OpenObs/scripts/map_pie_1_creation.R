...
for(i in name_map_df_pie_1){
  ...
  if(pie_2_valid | pie_3_valid | pie_4_valid){
    ...
    #print(unique(map_df_pie_1))
    #prot_geo <- addMarkers(prot_geo, lng=d$lng[1], lat=d$lat[1], layerId = paste(substring(i,14),map_pie_1_value,sep = " - "),popup = na.omit(as.character(unique(map_df_pie_1))), label = na.omit(as.character(levels(map_df_pie_1))))#
    ...
    #print(paste0(paste(substring(i,14),map_pie_1_value,sep = " - "),",",d$lng[1],",", d$lat[1],",", Freq))
  }
}