...
for(i in name_map_df_pie_3){
  ...
  if (pie_1_valid | pie_2_valid | pie_4_valid){
    ...
    prot_geo <- addMinicharts(map = prot_geo, lng = d$lng[1],lat = d$lat[1], type = "pie", chartdata = Freq,colorPalette = pal_3,width = 75,showLabels = TRUE,opacity = 0.7,labelMinSize = 1,labelMaxSize = 32,layerId =  paste(substring(i,14),map_pie_3_value,sep = " - "),popup = popupArgs(labels = levels(map_df_pie_3)),legend = FALSE)
  }
}